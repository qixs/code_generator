package com.qxs.plugin.generator.mybatis.plus.lombok.business.controller;
 ...
 */
public class MybatisPlusLombokBusinessControllerGenerator extends AbstractGenerator implements IGenerator {

	private static final String ENTITY_PLUGIN_NAME = "entity";

	private static final String BUSINESS_PLUGIN_NAME = "business";
	...
	protected TemplateModel wrap(PluginConfig[] allPluginConfigs,PluginConfig pluginConfig, Table table, String[] removePrefixs)
		...
		PluginConfig businessPluginConfig = getPluginConfig(allPluginConfigs, pluginConfig.getGroupName(), BUSINESS_PLUGIN_NAME);
		if(null == businessPluginConfig) {
			throw new PluginNotFoundException("插件[" + BUSINESS_PLUGIN_NAME + "]未找到");
		}
		businessClassName = businessPluginConfig.getPrefix() + businessClassName + businessPluginConfig.getSuffix();
		String businessClassParameterName = className.substring(0, 1).toLowerCase() + className.substring(1) + businessPluginConfig.getSuffix();

		map.put("businessClassName", businessClassName);

		map.put("businessClassParameterName", businessClassParameterName);
