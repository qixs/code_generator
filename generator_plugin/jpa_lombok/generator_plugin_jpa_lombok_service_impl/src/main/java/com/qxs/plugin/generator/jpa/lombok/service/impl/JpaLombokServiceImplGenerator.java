package com.qxs.plugin.generator.jpa.lombok.service.impl;
 ...
 */
public class JpaLombokServiceImplGenerator extends AbstractGenerator implements IGenerator {
    ...
    private static final String REPOSITORY_PLUGIN_NAME = "repository";
    ...
    protected TemplateModel wrap(PluginConfig[] allPluginConfigs,PluginConfig pluginConfig, Table table, String[] removePrefixs)
        ...
        //dao插件
        String repositoryClassName = className;
        String repositoryClassParameterName = repositoryClassName;

        PluginConfig repositoryPluginConfig = getPluginConfig(allPluginConfigs, pluginConfig.getGroupName(), REPOSITORY_PLUGIN_NAME);
        if(null == repositoryPluginConfig) {
            throw new PluginNotFoundException("插件["+REPOSITORY_PLUGIN_NAME+"]未找到");
        }
        repositoryClassName = repositoryPluginConfig.getPrefix() + className + repositoryPluginConfig.getSuffix();
        repositoryClassParameterName = className + repositoryPluginConfig.getSuffix();

        map.put("repositoryClassName", repositoryClassName);
        map.put("repositoryClassParameterName", repositoryClassParameterName.substring(0, 1).toLowerCase() + repositoryClassParameterName.substring(1));
